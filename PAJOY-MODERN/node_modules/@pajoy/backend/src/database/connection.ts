...
import { DATABASE_CONFIG } from '@pajoy/constants';
...
export const connectDatabase = async (): Promise<void> => {
  ...
  try {
    ...
    logger.info(`Connected to ${config.type} database successfully`);
  } catch (error) {
...
const connectPostgreSQL = async (config: DatabaseConfig): Promise<void> => {
  const poolConfig: PoolConfig = {
    ...
    max: DATABASE_CONFIG.POSTGRESQL.MAX_CONNECTIONS,
    connectionTimeoutMillis: DATABASE_CONFIG.POSTGRESQL.CONNECTION_TIMEOUT,
    idleTimeoutMillis: DATABASE_CONFIG.POSTGRESQL.IDLE_TIMEOUT,
  };
...
const connectSQLite = async (config: DatabaseConfig): Promise<void> => {
...
const runPostgreSQLMigrations = async (): Promise<void> => {
  const migrations = [
    `
...
const runSQLiteMigrations = async (): Promise<void> => {
  const migrations = [
    `
      CREATE TABLE IF NOT EXISTS users (
        id TEXT PRIMARY KEY,
